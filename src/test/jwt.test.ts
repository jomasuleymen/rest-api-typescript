...
describe("JWT", () => {
    ...
    test("should return a expired error", () => {
        expect(verifyToken(expiredToken, TokenType.ACCESS)).toEqual({
            decoded: null,
            expired: true,
            message: "jwt expired",
        });
    ...
    test("should return a malformed error", () => {
        expect(verifyToken("asdasd", TokenType.ACCESS)).toEqual({
            decoded: null,
            expired: false,
    ...
    test("should return a invalid token error", () => {
        expect(verifyToken(invalidToken, TokenType.ACCESS)).toEqual({
            decoded: null,
            expired: false,
    ...
    test("should verify token without any errors", () => {
        const { decoded, expired } = verifyToken(token, TokenType.ACCESS);
